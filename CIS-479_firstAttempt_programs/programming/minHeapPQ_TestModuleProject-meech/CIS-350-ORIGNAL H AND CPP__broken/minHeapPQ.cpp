...
void minHeapPQ<T>::insert(T insertElement) {
	...
	if (heapArray.size() == 1) { //case: first element to be added to queue; simply add it and exit function
		...
		parent = currentPos / 2;			//acquire and keep track of the parent of the inserted element
		...
		while (heapArray.at(currentPos) < heapArray.at(parent)) {	//check: is the parent greater than the child? if so: swap the parent and child so that the smaller child is moved up in the queue / PQ tree
			...
			currentPos--;									//current position just shifted up the tree (thus it now is at a lower element position in the array
			parent = currentPos / 2;						//update the new currentPosition of the inserted element with the its new parent location for use if another loop is necessary
...
T minHeapPQ<T>::deleteMin(void) {
	...
	default:
		while (heapArray[currentPos] > heapArray[leftChild] || heapArray[currentPos] > heapArray[rightChild]) {

			//if left < right, assign swap element that left value, otherwise assign it the right value (the smallest between the two, which are both smaller than their parent):
			...
			if (heapArray.size() - 1 >= 2 * currentPos) { leftChild = 2 * currentPos; }
			else { leftChild = 1; }

			if (heapArray.size() - 1 >= (2 * currentPos) + 1) { rightChild = (2 * currentPos) + 1; }
			else { rightChild = 1; }
			

			//case: no more childre for the currentPos to compare/swap with; tree balanced; exit loop
