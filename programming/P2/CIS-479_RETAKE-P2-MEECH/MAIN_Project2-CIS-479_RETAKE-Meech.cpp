//MAIN_Project2-CIS-479_RETAKE-Meech.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
//--> P(sense_obstacle|there is an obstacle)=0.8 + P(no_sense_obstacle|there is an obstacle)=0.2 = 1
const float sensing_OBJ_correctly_probability = (float)0.80; //correctly sense obstacle square as an obstacle = 80%
const float sensing_OBJ_incorrectly_probability = (float)0.20; //false negative(claiming no obstacle) - incorrectly sense square as NOT an obstacle (even though it is an obstacle) = 20%

//open-square (non-obstacle) sensor transition probabilities:
//-->They are complimentary --> should add up to 1.
//--> P(sense_obstacle|there is NOT an obstacle)=0.15 + P(no_sense_obstacle|there is NOT an obstacle)=0.85 = 1
const float sensing_no_OBJ_correctly_probability = (float)0.85; //correctly sense open square as NOT an obstacle = 85%
const float sensing_no_OBJ_incorrectly_probability = (float)0.15; //false positive(claiming an obstacle) - incorrectly sense open square as an obstacle (even though there is not an obstacle) = 15%
...
Location evidenceArray_WNES[4] = { OPEN_SQUARE }; //initialize array with all 0s = OPEN_SQUARE
...
int main()
{
    cout
        ...
        << "\nFor CIS-479_RETAKE with Doctor Shengquan Wang.\nWritten By: Demetrius Johnson (Meech)"
        << "\nWritten 10-28-2022 to 10-28-2022 (adapted from my previous program which took 9 days to write)";


//initialize location probability values:
    //note --> there are 6 rows (i) and 7 columns (j), and a total of 4 obstacles on the grid world maze.
    ...
    //6 x 7 = 42 total squares; 42 - 4 --> total squares - obstacle sqaures = 38 possible locations the robot could be at initialization; 
    //thus 1/38 = 0.0263 == 2.63%  --> it is in any 1 of the open 38 squares.
    //So, we initialize all non-obstacles squares wtih (1/38)
    ...
    for (int i = 0; i < table_struct_global.numRows; i++) {
        ...
        for (int j = 0; j < table_struct_global.numCols; j++) {
            ...
            table_struct_global.tablePos_locationProb_prior[i][j] = ( (float)1 / (float)38 ); //need to cast as a float (place 1 and 38 in registers as floats) to do floating point division
            table_struct_global.tablePos_locationProb_posterior[i][j] = ((float)1 / (float)38);
        }
    }
    ...
    */
    ...
    table_struct_global.tablePos_locationProb_prior[1][1] = obstacleVal;
    table_struct_global.tablePos_locationProb_prior[1][4] = obstacleVal;
    ...
    table_struct_global.tablePos_locationProb_posterior[1][1] = obstacleVal;
    table_struct_global.tablePos_locationProb_posterior[1][4] = obstacleVal;
    ...
    //Perform prediction calculations to update probability based on the move-North ACTION:
    //Afterwards, update prior as = calculated posterior, for use in the next posterior calculation
    ...
    //We can perform as many updates as we like and get closer
        //and closer (with some limitations due to uncertainty of sensors and movement) 
        //to a convergence value at a location == robot's (most likely) location:
    ...
    for (int row = 0; row < table_struct.numRows; row++) {
        for (int col = 0; col < table_struct.numCols; col++) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                //case: incorrectly sense an obstacle as open square (false negative)  ==20%
                ...
                //case: incorrectly sense an open sqaure as an obstacle (false positive) ==15%
